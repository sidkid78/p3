...
const Sidebar = ({ isOpen, onClose }: { isOpen?: boolean; onClose?: () => void }) => {
  ...
  const navItems = [
    ...
    { icon: 'sports_esports', label: 'Game Planner', page: Page.GamePlanner },
    { icon: 'photo_library', label: 'Photo Gallery', page: Page.Vendors },
    { icon: 'send', label: 'Digital Invitation', page: Page.Invitation },
    { icon: 'redeem', label: 'Gift Registry', page: Page.GiftRegistry },
    { icon: 'edit_note', label: 'Invitation Editor', page: Page.InvitationEditor },
    { icon: 'lock', label: 'Password Gate', page: Page.InvitationPasswordGate },
    { icon: 'history_edu', label: 'Thank You Notes', page: Page.AIThankYouHelper },
  ];
  ...
  return (
    <>
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 flex flex-col glass-panel h-full p-6 overflow-y-auto border-r border-panel-border transition-all duration-500 ease-in-out z-50 lg:relative lg:translate-x-0 ${isOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'
        }`}>
